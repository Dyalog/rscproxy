...
#define strncpyLimit (buffersize - 1)
  static char lsRVersionMajor[buffersize];
#else
  ...
  {
#if defined(__WINDOWS__)
    lsRVersionMajor[strncpyLimit] = 0; // trailing null to prevent scan overruns.
    lsVersionInit = 1;
    strncpy(lsRVersionMajor, getDLLVersion(), strncpyLimit);
...
/* 08-05-04 | baier | reworked code for adding driver */
